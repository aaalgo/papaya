...
def Papaya (path, input_path, annotations=Annotations(), images = None, text = ''):
    try:
        os.makedirs(path)
    except:
        pass
    ...
    subprocess.check_call('%s/dcm2niix -z i -o %s -f case %s' % (BASE, path, input_path), shell=True)
    boxes = []
    centers = []
...
if __name__ == '__main__':
    ...
    Papaya(args.output[0], args.input[0])
